...
- [x] Introduce a hierarchical battle state machine for rounds, phases, and phase substeps.
  - Completed: shared round/phase graph, movement substeps, five-round boundary, typed transition events, phase-owned entry-state handlers, active-phase legal-action/completion handlers, Fight on Death interrupt ownership, and one shared orchestration path for full-phase and unit-step simulation.
...
- [x] Add Fight On Death support once datasheet/stratagem timing data exists; imported Fight On Death text now opens a serialized per-model interrupt window with target/weapon selection, decline handling, legal-action gating, replay/save support, and deferred damage resolution.
...
- [ ] Add exact 11th command abilities once datasheet/ability source data exists; core currently supports command-phase timing but does not invent generic ability effects.
  - Progress: automatic, unconditional imported text effects now cover exactly 1CP gains and exactly 1 lost-wound recovery at Command; conditional rolls, selectable choices, resurrection, resources, and detachment-specific effects remain fail-closed.
  - Progress: unconditional imported self-model "regains up to D3 lost wounds" Command abilities now resolve the D3 recovery; conditional, selectable, multi-model, and resurrection effects remain fail-closed.
  - Progress: imported Command abilities that explicitly keep a controlled objective secured now use the existing 11th-edition secured-objective state when their unit is in range.
...
- [ ] Add/finalize 11th Core 25 Muster Armies in the army-builder/import layer: current core can use imported rosters, but full faction, battle size, roster construction, and validation rules need official source/data.
  - Progress: portable roster validation now rejects malformed army metadata, optional saves, roster IDs, deployment modes, transport capacities, per-model weapon loadout references, model stat profiles, model-base geometry, movement overrides, damaged profiles, weapon profiles, rule text, keyword lists, ambiguous deployment/attachment relationships, and malformed catalog/options containers or IDs. BattleScribe imports now preserve authoritative roster points, the selected battle-size ID and limits, and all selected army/detachment rule text as catalog data; Army Builder validation consumes that catalog automatically. Full faction catalogs, faction limits, force dispositions, and Leader-joining data remain source-dependent. The official Games Workshop downloads page links the Munitorum Field Manual at https://mfm.warhammer-community.com/ for those remaining data.
...
- [ ] Re-audit shared 10th-compatible movement, shooting, charge, fight, Battle-shock, objective control, reserves, transports, terrain, aircraft, and vertical rules against the final full 11th wording.
  ...
  - Progress: final Core 13.09 Hidden now tracks current/previous-turn ranged attacks and limits quiet Infantry/Beasts/Swarm visibility to the 15-inch detection range inside areas containing light/dense features; Core 13.11.01 Gone to Ground reduces that detection range to 12 inches only when dense terrain conceals the full model footprint. Terrain features now carry explicit light/dense categories with deterministic inference for legacy layouts; exact source tagging for imported Event Companion layouts remains open.
...
- [ ] Continue datasheet/character ability support beyond the first passive hooks: Leader aura/attached-unit modifiers, Precision interactions beyond targeting, and faction-specific abilities as source text/data becomes available.
  - Progress: imported ranged-save text such as Snikrot's "+1 Sv against ranged attacks" now improves only ranged saves, including attached-unit resolution; unsupported ability wording remains inert.
