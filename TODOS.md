...
- [ ] Add state tracking for unsupported mission event clauses: remaining objective actions and territory geometry. Objective owners, objective proximity, terrain-area membership, table-quarter presence, destroying-unit kill-time proximity, completed mission actions, persistent objective/terrain operation markers, central/expansion objective roles, condemned-unit selections, and battlefield exits are tracked.
...
- [ ] Add UI controls for declaring split melee attack counts across multiple engaged targets. Core support exists, but current app controls still use the single-target fight path.
...
- [ ] Add destroyed model/unit last-position tracking if a later rule or mission needs measuring to a destroyed model after it has been removed from the battlefield.
...
- [ ] Re-check exact 11th Overrun Fight wording once the full official source is available; current core supports charged units piling in to become able to fight, but does not add a separate named Overrun state.
- [ ] Re-check exact 11th Secured Objectives wording once the full official source is available; current core tracks terrain objective owners from current OC each scoring check and does not add sticky-control behavior without source confirmation.
- [ ] Add/finalize 11th Core 19 Attached Units beyond current targeting: forming attached units is partially represented during deployment/import, bodyguard protection, Precision targeting, and attached-unit keyword sharing for Anti-style wound checks are covered; leader/bodyguard destruction split state and attached-unit ability sharing need exact source/data.
- [ ] Add/finalize 11th Core 21 Flying and Surging beyond current Fly movement: Fly movement/vertical interactions are covered, but Surge Moves need exact trigger/timing/source data.
- [ ] Add/finalize 11th Core 22 Other Rules and Abilities: Aura, faction, Psychic, wargear, and Plunging Fire need datasheet/source data and exact final wording before adding generic behavior.
