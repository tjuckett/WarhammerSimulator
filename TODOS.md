...
- [x] Re-run `npx tsc -p apps/web/tsconfig.json --noEmit`, `npm run lint`, and root `npm run build` after frontend/rules changes. Latest verification passes; lint retains 3 React Hook dependency warnings in render-local handlers/effects that need a broader ref pattern or callback reordering, and no errors.
