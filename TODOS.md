...
- [x] Add UI controls for declaring split melee attack counts across multiple engaged targets. The Fight panel keeps the single-target flow when only one target is legal, and for fixed-Attacks melee profiles exposes editable per-target allocations with running totals, clear/reset controls, and core validation. Split declarations are serialized on the existing fight action for deterministic undo/replay; variable-Attacks profiles continue through the normal single-target resolution because their attack total is not known before rolling.
